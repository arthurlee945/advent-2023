function gameD2(games: string[]) {
    return games.reduce((powSum, game) => {
        const minRGB = [0, 0, 0];
        const [_, bagSets] = game.split(": ");
        const gSets = bagSets.split(/[;|,]\s/);
        for (const dice of gSets) {
            const [count, color] = dice.split(" ");
            if (color === "red" && minRGB[0] < +count) minRGB[0] = +count;
            else if (color === "green" && minRGB[1] < +count) minRGB[1] = +count;
            else if (color === "blue" && minRGB[2] < +count) minRGB[2] = +count;
        }
        powSum += minRGB[0] * minRGB[1] * minRGB[2];
        return powSum;
    }, 0);
}
const D2Test = `Game 1: 1 green, 2 blue; 15 blue, 12 red, 2 green; 4 red, 6 blue; 10 blue, 8 red; 3 red, 12 blue; 1 green, 12 red, 8 blue
...
console.log(gameD2(D2Test.split("\n")));
