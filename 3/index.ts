...
const ruleSet: rgb = [12, 13, 14];
function gameSets(gameStr: string): boolean {
    const gSets = gameStr.split("; ").map((g) => g.split(", "));
    for (const g of gSets) {
        const impossible = g.find((dice) => {
            const [count, color] = dice.split(" ");
            if (color === "red") return ruleSet[0] < +count;
            else if (color === "green") return ruleSet[1] < +count;
            else if (color === "blue") return ruleSet[2] < +count;
        });
...
function game(games: string[]) {
    let idSum = 0;
    for (const game of games) {
        const [gameName, bagSets] = game.split(": ");
        const gameId = gameName.split(" ")[1];
        if (gameSets(bagSets) && !isNaN(+gameId)) idSum += +gameId;
    }
    return idSum;
}

const testGames2 = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`;

const testGames = `Game 1: 1 green, 2 blue; 15 blue, 12 red, 2 green; 4 red, 6 blue; 10 blue, 8 red; 3 red, 12 blue; 1 green, 12 red, 8 blue
...
console.log(game(testGames.split("\n")));
