// function D3(schema: string) {
//     const cleanSchema = schema.split("\n").join("");
//     const dir = [-11, -10, -9, -1, 1, 9, 10, 11];
//     return Object.values(
//         cleanSchema.split("").reduce((currSet, str, strIndex, set) => {
//             if (/[0-9]/.test(str) || str === ".") return currSet;
//             const posToCheck = dir.reduce((ptc, curr) => {
//                 if (strIndex + curr < 0) return ptc;
//                 ptc.push(strIndex + curr);
//                 return ptc;
//             }, [] as number[]);
//             for (const pos of posToCheck) {
//                 if (!/[0-9]/.test(set[pos])) continue;
//                 const leftMatch = cleanSchema.slice(0, pos + 1).match(/\d+$/)?.[0];
//                 const rightMatch = cleanSchema.slice(pos + 1).match(/^\d+/)?.[0];
//                 if ((!leftMatch && !rightMatch) || currSet[pos - (leftMatch?.length ?? 0) + 1]) continue;
//                 currSet[pos - (leftMatch?.length ?? 0) + 1] = +((leftMatch ?? "") + (rightMatch ?? ""));
//             }
//             return currSet;
...
function D3(schema: string[]) {
    ...
    return Object.values(
        schema.reduce((curr, rowSet, rowIndex, schemaSet) => {
            for (let i = 0; i < rowSet.length; i++) {
                if (/[0-9]/.test(rowSet[i]) || rowSet[i] === ".") continue;
                const posToCheck = dir.reduce((ptc, curr) => {
                    if (i - curr[0] < 0 || rowIndex - curr[1] < 0) return ptc;
                    ptc.push([i - curr[0], rowIndex - curr[1]]);
                    return ptc;
                }, [] as [number, number][]);
                for (const pos of posToCheck) {
                    if (!/[0-9]/.test(schemaSet[pos[1]][pos[0]])) continue;
                    const leftMatch = schemaSet[pos[1]].slice(0, pos[0] + 1).match(/\d+$/)?.[0];
                    const rightMatch = schemaSet[pos[1]].slice(pos[0] + 1).match(/^\d+/)?.[0];
                    if ((!leftMatch && !rightMatch) || curr[`${pos[1]}-${pos[0] - (leftMatch?.length ?? 0) + 1}`]) continue;
                    curr[`${pos[1]}-${pos[0] - (leftMatch?.length ?? 0) + 1}`] = +((leftMatch ?? "") + (rightMatch ?? ""));
                }
            }
...
const sampleTest = `....................................18..........889.270.....748.280...997.................617..............622........763............476....
...
console.log(D3(sampleTest.split("\n")));
