...
const digitKey = new Set(digits.map((d) => d[0]));
const digitMap = digits.reduce((map, d, i) => {
    map[d] = i + 1;
    return map;
}, {} as Record<(typeof digits)[number], number>);

const calibrate2 = (codes: string[]): number =>
    codes.reduce((calVal, code) => {
        const val: [number | null, number | null] = [null, null];
        const codeArr = code.split("");
        for (let i = 0; i < codeArr.length; i++) {
            const fStrToCheck = codeArr[i];
            if (!val[0] && (!isNaN(+fStrToCheck) || digitKey.has(fStrToCheck))) {
                if (isNaN(+fStrToCheck)) {
                    const found = digits.find((d) => code.slice(i).startsWith(d));
                    val[0] = found ? digitMap[found] : null;
                } else {
                    val[0] = +fStrToCheck;
                }
            }

            const lStrToCheck = codeArr[codeArr.length - 1 - i];
            if (!val[1] && (!isNaN(+lStrToCheck) || digitKey.has(lStrToCheck))) {
                if (isNaN(+lStrToCheck)) {
                    const found = digits.find((d) => code.slice(codeArr.length - 1 - i).startsWith(d));
                    val[1] = found ? digitMap[found] : null;
                } else val[1] = +lStrToCheck;
            }

            if (val[0] && val[1]) return (calVal += val[0] * 10 + val[1]);
        }

        return calVal;
    }, 0);

const codeStr2 = `two934seven1
8825eightknfv
sevenoneqbfzntsix55
...
console.log(calibrate2(codeStr2.toLowerCase().split("\n")));
