    ...
    return Object.values(
        schema.reduce((curr, rowSet, rowIndex, schemaSet) => {
            for (let i = 0; i < rowSet.length; i++) {
                if (rowSet[i] !== "*" || curr[`${rowIndex}-${i}`]) continue;
                ...
                for (const pos of posToCheck) {
                    ...
                    if ((!leftMatch && !rightMatch) || tempGear.includes(+((leftMatch ?? "") + (rightMatch ?? "")))) continue;
                    tempGear.push(+((leftMatch ?? "") + (rightMatch ?? "")));
                }
                if (tempGear.length === 2) curr[`${rowIndex}-${i}`] = tempGear[0] * tempGear[1];
            }
