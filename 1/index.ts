const calibrate = (codes: string[]): number =>
    codes.reduce((calVal, code) => {
        const val: [number | null, number | null] = [null, null];
        const codeArr = code.split("");
        for (let i = 0; i < codeArr.length; i++) {
            if (!val[0] && !isNaN(+codeArr[i])) val[0] = +codeArr[i];
            if (!val[1] && !isNaN(+codeArr[codeArr.length - 1 - i])) val[1] = +codeArr[codeArr.length - 1 - i];
            if (val[0] && val[1]) return (calVal += val[0] * 10 + val[1]);
        }
        return calVal;
    }, 0);

const codeStr = `two934seven1
8825eightknfv
sevenoneqbfzntsix55
...
console.log(calibrate(codeStr.split("\n")));
